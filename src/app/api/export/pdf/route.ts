...
import { generateReviewerPdf } from "@/lib/pdf-export";
...
export async function GET(req: NextRequest) {
  ...
  const pdfBytes = generateReviewerPdf(
    {
      title: deck.title,
  ...
  const filename = `${deck.title.replace(/[^a-zA-Z0-9 ]/g, "").replace(/\s+/g, "_")}_Reviewer.pdf`;
